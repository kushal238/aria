...
import 'doctor_profile_completion_screen.dart'; // Import the doctor profile screen
     ...
     try {
        ...
        final response = await http.post(
          ...
          headers: {
            ...
            'X-App-Type': 'DOCTOR', // Add the required header for the doctor app
          },
        ...
        if (response.statusCode == 200) {
          ...
          if (apiToken != null && userProfile != null) {
            ...
            // Check for the doctor-specific profile and its status.
            final Map<String, dynamic>? doctorProfile = userProfile['doctor_profile'];
            final String doctorStatus = doctorProfile?['status'] ?? 'PROFILE_INCOMPLETE';

            if (doctorStatus == 'PROFILE_INCOMPLETE') {
              print('Doctor profile is incomplete. Navigating to DoctorProfileCompletionScreen.');
              Navigator.of(context).push(
                MaterialPageRoute(builder: (context) => const DoctorProfileCompletionScreen()),
              );
            } else {
              print('Doctor profile is complete. Navigating to HomeScreen.');
